...
describe("Testing the LocationBlog Facade", function () {
  ...
  beforeEach(async function () {
    ...
    position = await Promise.all([
        new Position({ user: users[1]._id, loc: { type: 'Point', coordinates: [1.12, 2.21] } }).save(),
        new Position({ user: users[0]._id, loc: { type: 'Point', coordinates: [3.32, 1.44] } }).save()
    ])
  })
  ...
  it("getByUsername: Should find the right positions from given usernames", async function () {
    ...
    expect(position.loc.coordinates[0]).to.be.equal(3.32);
    position = await positionFacade.getByUsername('hw');
    expect(position.loc.coordinates[0]).to.be.equal(1.12);
  });
  ...
  it("updateOrCreate: Should update the position with new coordinates given user and coordinates", async function () {
    ...
    const position = await positionFacade.updateOrCreate(user, longitude, latitude)
